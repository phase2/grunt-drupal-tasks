module.exports = function(grunt) {
   ...
   */
  module.eventify = function(scripts, task, namespace) {
    if (!scripts) {
      return;
    }
    ...
    var commands = {};
    if (scripts['pre-' + task]) {
      command.pre = buildCmd('pre-' + namespace, scripts['pre-' + task]);
    }
    if (scripts['post-' + task]) {
      command.post = buildCmd('post-' + namespace, scripts['post-' + task]);
    }
